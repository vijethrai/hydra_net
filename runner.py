...
def train_model(model_type,param_vals,_trainX_Stream,_trainY_Stream,_testX_Stream,_testY_Stream,_valX_Stream,_valY_Stream)   :
     ...
     cnt = 0; current_seq_length =0;
     #print ("# of Param Sets", len(product(*param_vals.values())))
     for p_tups in product(*param_vals.values()):
                        ...
                        if   current_seq_length != model_params['seq_length']    :
                             print (" Generate data only if seq length parameter changed %d to %d "%(current_seq_length, model_params['seq_length' ]))
                             pred_window=0; 
                             ...
                             _valX,_valY=dm.generateWindows(_valX_Stream,_valY_Stream,model_params['seq_length'],pred_window)     
                             _trainX,_trainY,_testX,_testY,_valX,_valY=dm.preProcessData(_trainX,_trainY,_testX,_testY,_valX,_valY)
                             current_seq_length = model_params['seq_length']
                        ...
                        for iter_trial in range(num_trials)   :                                                            
                             ...
                             print ("\n Training input shapes",(_trainX.shape,_trainY.shape,_testX.shape,_testY.shape,_valX.shape,_valY.shape))
                             this_trial_predictions,this_trial_error=nn_model.train_data(_trainX,_trainY,_testX,_testY,_valX,_valY)
